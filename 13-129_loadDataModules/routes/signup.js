...
//require Firebase 註冊登入資訊 (13-131)
var firebase = require('../connections/firebase_connect');
//require FirebaseDB 資訊 (13-132)
...
router.post('/', function (req, res) {
    //從signup.ejs post過的資訊 (13-131)
    var email = req.body.email;
    var password = req.body.password;

    //由於有可能 Auth 回傳user的資訊是不足的，有可能我自需要是否結婚資訊， 暱稱是什麼資訊 (13-132)
    ...
    .then(function(user){
        ...
        //成功後會收到Auth回傳的user 的資料 (13-132)
        //裡面會有非常多資訊 ex UID:這個User專有的id, email, password, 是否已認證 等等
        console.log(user);

        //存放更多該User的額外資訊至firebaseDb (13-132)
        var saveUser = {
            'email': email,
            'nickname': nickname,
            'age': age,
        ...
        //成功後轉跳success 頁面 (13-131)
        res.redirect('/signup/success')
    })
    .catch(function(error) {
        //傳送失敗

        //如果再註冊失敗則會跳到catch(), 並回傳error 帶錯誤資訊 (13-133)
